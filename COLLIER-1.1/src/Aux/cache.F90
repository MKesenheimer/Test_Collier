...
!        end if
  ...
  subroutine InitCache_cll(ncache_in)
    ...
    ncache = ncache_in
    !print*,"ncache",ncache,"nevent",nevent(ncache),"nopt",nopt(ncache) ! MK, debug
      ...
      if(use_cache(ncache).ge.N) then
        ...
        select case(cache_mode(ncache))
          ...
          case(0)
            ! fully optimized run
            ! MK:
            if(size(CachePoint,1).lt.id) then
              print*,"error in Collier.ReadCache: id, invalid memory reference."
              print*,"size(CachePoint,1) = ",size(CachePoint,1)
              print*,"argument(1)        = ",id
              stop
            endif
            if(size(CachePoint,2).lt.ncall) then
              print*,"error in Collier.ReadCache: ncall, invalid memory reference."
              print*,"size(CachePoint,2) = ",size(CachePoint,2)
              print*,"argument(2)        = ",ncall
              stop
            endif
            if(size(CachePoint,3).lt.ncache) then
              print*,"error in Collier.ReadCache: ncache, invalid memory reference."
              print*,"size(CachePoint,3) = ",size(CachePoint,3)
              print*,"argument(3)        = ",ncache
              stop
            endif
            print*,"ncall = ",ncall,"cache_mode",cache_mode(ncache),"size(CachePoint)",size(CachePoint,2) ! MK, debug
